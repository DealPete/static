...
fn decode_regular_inst(buffer: &[u8], offset: usize) -> Result<Instruction, String> {
	...
	match code & 7 {
		...
		5 => 
			...
			inst.op2 = Some(Operand::Imm16(get_word_le(&buffer, offset + 1) as i16));
			inst.length = 3;
			},
...
fn decode_mov_moffset(buffer: &[u8], offset: usize) -> Instruction {
    ...
    let op2 = Some(Operand::Pointer(
        Pointer::new(size, PtrType::Disp16(get_word_le(buffer, offset + 1)))));
    if code & 2 != 0 {
        inst.op1 = op2;
        inst.op2 = op1;
    } else {
...
fn decode_test_ax(buffer: &[u8], offset:usize) -> Result<Instruction, String> {
    ...
    inst.op2 = Some(match size {
        ...
        _ => Operand::Imm16(get_word_le(buffer, offset + 1) as i16),
    });
    inst.length = 2 + size;
    return Ok(inst);
}
...
fn decode_call_jump_rel16(buffer: &[u8], offset: usize) -> Instruction {
    ...
    inst.op1 = Some(Operand::Imm16(get_word_le(buffer, offset + 1) as i16));
    inst.length = 3;
    return inst;
}
    ...
    let (r_op1, r_op2, r_length) = match mode {
        0 => match op2
            ...
            6 => (reg(op1), Operand::Pointer(
                Pointer::new(size, PtrType::Disp16(get_word_le(&buffer, offset + 1)))), 3),
        ...
        2 => (reg(op1), match op2 {
            0 => Operand::Pointer(Pointer::new(size,
                PtrType::RegRegDisp16(Register::BX, Register::SI, get_word_le(&buffer, offset + 1)))),
            1 => Operand::Pointer(Pointer::new(size,
                PtrType::RegRegDisp16(Register::BX, Register::DI, get_word_le(&buffer, offset + 1)))),
            2 => Operand::Pointer(Pointer::new(size,
                PtrType::RegRegDisp16(Register::BP, Register::SI, get_word_le(&buffer, offset + 1)))),
            3 => Operand::Pointer(Pointer::new(size,
                PtrType::RegRegDisp16(Register::BP, Register::DI, get_word_le(&buffer, offset + 1)))),
            4 => Operand::Pointer(Pointer::new(size,
                PtrType::RegDisp16(Register::SI, get_word_le(&buffer, offset + 1)))),
            5 => Operand::Pointer(Pointer::new(size,
                PtrType::RegDisp16(Register::DI, get_word_le(&buffer, offset + 1)))),
            6 => Operand::Pointer(Pointer::new(size,
                PtrType::RegDisp16(Register::BP, get_word_le(&buffer, offset + 1)))),
            7 => Operand::Pointer(Pointer::new(size,
                PtrType::RegDisp16(Register::BX, get_word_le(&buffer, offset + 1)))),
            _ => panic!("Unknown r/m bits!")
            }, 3),
        3 => (reg(op1), reg(op2), 1),
