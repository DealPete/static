...
static PRELUDE: &str =
"#include \"chip8/src/display.h\"
#include \"chip8/src/input.h\"
#include <stdint.h>
...
fn main() {
	...
	if let Some(arg) = env::args().nth(1) {
        ...
        match result {
            Ok(graph) => {
                let filename = format!("{}.c", stem);
                let mut file = File::create(filename.as_str())
                    .expect("Couldn't create output file.");

                let source = source_string(graph, stem);
...
fn source_string(graph: FlowGraph<Instruction>, file_stem: &str) -> String {
    let mut output = format!("{}\tinit_window(\"{}\");\n\n", PRELUDE, file_stem);
    ...
    output.push_str(format!("{}", POSTLUDE).as_str());
