...
pub trait RangeTrait<T: Add + Ord + Copy, R: RangeTrait<T, R>> {
    fn new() -> R;
...
impl<T: Add + Ord + Copy> RangeTrait<T, RangeVec<T>> for RangeVec<T> {
    fn new() -> RangeVec<T> {
        Vec::new()
    }
...
/*
impl RangeTrait<usize, RangeVec<usize>> for RangeVec<usize> {
    fn new() -> RangeVec<usize> {
        Vec::new()
    }

    fn from_range(min: usize, max: usize) -> RangeVec<usize> {
        vec!(Range { min, max })
    }

    fn insert(self, value: usize) -> RangeVec<usize> {
        self.insert_range(value, value)
    }

    fn insert_range(self, min: usize, max: usize) -> RangeVec<usize> {
        self.combine(RangeVec::from_range(min, max))
    }
    ...
    fn combine(self, range: RangeVec<usize>) -> RangeVec<usize> {
        if self.len() == 0 {
            return range;
        }
